...
<head>
  ...
  <?php if(!empty( $_GET['dproject_name'])) $page = $_GET['dproject_name'] ?>
    ...
    $ordr=$_POST['ordr'.$i];
     ...
     if (!empty($_FILES['image1']['name'])) {
       ...
       $target = "../images/products/".basename($image);
           ...
           $data=mysqli_query($con,"UPDATE product SET `img`='$image' where `id`=$id")or die( mysqli_error($con) );
     ...
     $sql = "UPDATE product SET `name` = '$name',`slug` = '$slug',`code` = '$code',`price` = '$price',`desp` = '$desp',`metak` = '$metak',`metad` = '$metad',`ordr` = '$ordr',`feat` = '$feat' WHERE `id` =$id";
        ...
        <div class="col-lg-10">
          <div class="widget-container fluid-height clearfix">
            <div class="heading" style="text-transform: capitalize;">
              <i class="fa fa-lightbulb-o"></i> <?php echo $page ?> Project Details
              <span style="float: right;    text-decoration: underline;">
                <?php

                $rows =mysqli_query($con,"SELECT pslug FROM product where slug='$page'  ORDER BY ordr" ) or die(mysqli_error($con));
                $n=0;
                ...
                while($row=mysqli_fetch_array($rows)){
                  ...
                  ?>
              <a href="jewelrys-<?php echo $pslug ?>"> <i class="fa fa-reply"></i>Back to Category </a>
            </span>
            </div>
